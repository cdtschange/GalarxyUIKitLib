...
OSStatus MyEnqueueBuffer(OLAudioStreamer* myData);
...
{	
	...
	OLAudioStreamer* streamer = (OLAudioStreamer *)inClientData;
	[streamer
...
void MyAudioQueueIsRunningCallback(void *inUserData, AudioQueueRef inAQ, AudioQueuePropertyID inID)
{
	OLAudioStreamer* streamer = (OLAudioStreamer *)inUserData;
...
{
	OLAudioStreamer* streamer = (OLAudioStreamer *)inClientInfo;
...
@implementation OLAudioStreamer
...
- (void)failWithErrorCode:(AudioStreamerErrorCode)anErrorCode
{
	@synchronized(self)
	{
		...
		if (err)
		{
            ...
            if (![self isFinishing]) {
                NSLog(@"%@ err: %c%c%c%c %d\n",
                      [OLAudioStreamer stringForErrorCode:anErrorCode],
		...
		{
			NSLog(@"%@", [OLAudioStreamer stringForErrorCode:anErrorCode]);
		}
	...
	else if (eventType == kCFStreamEventHasBytesAvailable)
	{
		...
		if (!audioFileStream)
		{
			...
			AudioFileTypeID fileTypeHint =
				[OLAudioStreamer hintForFileExtension:[[url path] pathExtension]];

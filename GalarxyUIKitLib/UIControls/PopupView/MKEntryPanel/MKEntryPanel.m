...
- (id)initWithParent:(UIView*) aParentView onTappedSelector:(SEL) tappedSel
{
    ...
    if (self) {
        ...
        frame.size.width = aParentView.frame.size.width;
        self.frame =frame;
...
+(MKEntryPanel*) panel
{
    MKEntryPanel *panel =  (MKEntryPanel*) [[[UINib nibWithNibName:@"MKEntryPanel" bundle:RESOURCEBUNDLE]
                                           instantiateWithOwner:self options:nil] objectAtIndex:0];

    
    panel.backgroundGradient.image = [UIIMAGERESOURCEBUNDLE(@"mkentry_TopBar",@"png") stretchableImageWithLeftCapWidth:1 topCapHeight:5];

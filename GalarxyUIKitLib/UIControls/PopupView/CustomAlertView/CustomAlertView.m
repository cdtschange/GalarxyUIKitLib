...
-(void)dealloc{
    self.alertViewBackground=nil;
    self.alertViewButtonCancel=nil;
    self.alertViewButtonOK=nil;
}
...
-(void)layoutSubviews{
    for (UIView *v in self.subviews) {
        if ([v isKindOfClass:[UIImageView class]]) {
            ...
            UIImage *image = self.alertViewBackground;
        ...
        if ([v isKindOfClass:NSClassFromString(@"UIAlertButton")]) {
            ...
            UIImage *image = nil;
            if (button.tag == 1) {
                image = self.alertViewButtonCancel;
            }else{
                image = self.alertViewButtonOK;
            }

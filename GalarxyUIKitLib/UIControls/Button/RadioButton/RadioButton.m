...
+(void)removeObserverForGroupId:(NSString*)groupId{
    if (groupId) {
        if (rb_observers &&[[rb_observers allKeys] containsObject:groupId]) {
            [rb_observers removeObjectForKey:groupId];
        }
        if (rb_instancesDic &&[[rb_instancesDic allKeys] containsObject:groupId]) {
            [rb_instancesDic removeObjectForKey:groupId];
        }
    }
}
...
-(void)defaultInit{
    ...
    [_button setImage:UIIMAGERESOURCEBUNDLE(@"RadioButton-Unselected",@"png") forState:UIControlStateNormal];
    [_button setImage:UIIMAGERESOURCEBUNDLE(@"RadioButton-Selected",@"png") forState:UIControlStateSelected];

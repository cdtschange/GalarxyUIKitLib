...
withImage:(UIImage *)image withHighlightImage:(UIImage *)highlightImage{
    ...
    if (self)
	{
		...
		self.minHandle = [[[UIImageView alloc] initWithImage:image highlightedImage:highlightImage] autorelease];
		self.minHandle.center = CGPointMake(sliderBarWidth * 0.2, sliderBarHeight * 0.5);
		[self addSubview:self.minHandle];
		
		self.maxHandle = [[[UIImageView alloc] initWithImage:image highlightedImage:highlightImage] autorelease];
...
+ (id) doubleSliderWithImage:(UIImage *)image withHighlightImage:(UIImage *)highlightImage
{
	return [[[self alloc] initWithFrame:CGRectMake(0., 0., 300., 40.) minValue:0.0 maxValue:100.0 barHeight:10.0 withImage:image withHighlightImage:highlightImage] autorelease];
}

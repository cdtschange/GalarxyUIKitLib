...
- (id)initWithStyle:(HZAreaPickerStyle)pickerStyle delegate:(id<HZAreaPickerDelegate>)delegate
{
    ...
    self = [[[RESOURCEBUNDLE loadNibNamed:@"HZAreaPickerView" owner:self options:nil] objectAtIndex:0] retain];
    if (self) {
        ...
        if (self.pickerStyle == HZAreaPickerWithStateAndCityAndDistrict) {
            provinces = [[NSArray alloc] initWithContentsOfFile:[RESOURCEBUNDLE pathForResource:@"hzarea_area.plist" ofType:nil]];
            ...
            provinces = [[NSArray alloc] initWithContentsOfFile:[RESOURCEBUNDLE pathForResource:@"hzarea_city.plist" ofType:nil]];
            cities = [[provinces objectAtIndex:0] objectForKey:@"cities"];
            self.locate.state = [[provinces objectAtIndex:0] objectForKey:@"state"];

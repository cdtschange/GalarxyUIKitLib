...
#import "HegakaGalleryScrollView.h"
#import "HegakaGalleryButton.h"

@implementation HegakaGalleryScrollView
...
- (void) awakeFromNib
{
    ...
    // RECYCLE BIN
    UIImageView *imageViewTemp = [[UIImageView alloc] initWithImage:UIIMAGERESOURCEBUNDLE(@"hegaka_recyclebin",@"png")];
...
- (void) addAttachment:(HegakaAttachmentItem *)attachment
{
    ...
    HegakaGalleryButton *btnAttachment = [[HegakaGalleryButton alloc] initWithFrame:CGRectMake(startX, startY, width, height)];
    ...
    if (attachment.type == 1){
        ...
        UIImageView *imageView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, 64, 64)];
        imageView.image=UIIMAGERESOURCEBUNDLE(@"iconSoundFile",@"png");
        [btnAttachment addSubview:imageView];
        [imageView release];
    } else if (attachment.type == 3){
        UIImageView *imageView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, 64, 64)];
        imageView.image=UIIMAGERESOURCEBUNDLE(@"iconVideoCamera",@"png");
        [btnAttachment addSubview:imageView];
        [imageView release];
    } 
...
- (void) removeAttachment:(HegakaGalleryButton *)button
{
    ...
    NSUInteger idx = 0;
    for (HegakaAttachmentItem *item in _attachments) {
        if (idx != position) {
...
- (void) reloadData
{
    ...
    for (int i=0; i<[_attachments count] ; i++) {
        HegakaAttachmentItem *item = [_attachments objectAtIndex:i];
        _scrollView.contentSize = CGSizeMake([_attachments count]*70, 70);    
